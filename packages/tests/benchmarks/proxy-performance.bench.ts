...
test("local proxy translation overhead", async () => {
  ...
  try {
    ...
    const result = {
      ...
      upstreamRequests,
      notes: [
        "ai& upstream is mocked, so this measures local proxy translation and forwarding overhead.",
...
test("large proxy in-process translation breakdown", async () => {
  ...
  const result = {
    ...
    notes: [
      ...
      "JSON time includes inbound request parse, upstream ai& request serialization, upstream response parse, and outbound client response serialization.",
    ],
  };
...
test("captured headless proxy payload breakdown", async () => {
  ...
  const result = {
    ...
    notes: [
      ...
      "The recorder saves the largest real inbound client payload and the benchmark replays it through the real aiandrelay proxy handlers.",
    ],
  };
...
test("high-volume streaming parser throughput", async () => {
  ...
  const result = {
    ...
    notes: [
      "Mocks one ai& streaming response with many SSE data events and calls the real Claude proxy handler in-process.",
...
test("vision delayed failover timing", async () => {
  ...
  const raceResult = await benchmarkVisionScenario(
    ...
    1,
    async () => {
      vi.stubEnv("AIANDRELAY_VISION_FAILOVER_RACE_DELAY_MS", "5");
      let requests = 0;
      vi.stubGlobal(
        "fetch",
        vi.fn(async (_url: string | URL | Request, init?: RequestInit) => {
...
function sanityP95CeilingMs(): number {
  const raw = process.env.AIANDRELAY_PROXY_BENCH_MAX_RAW_P95_MS;
  return raw ? Number.parseFloat(raw) : 100;
}

function optionalOverheadCeilingMs(): number | undefined {
  const raw = process.env.AIANDRELAY_PROXY_BENCH_MAX_P95_OVERHEAD_MS;
  if (!raw) {
    return undefined;
  }
