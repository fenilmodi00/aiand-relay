...
import { GLM_5_2, AIAND_BASE_URL } from "@aiandrelay/models";
...
const maybeTest = process.env.AIANDRELAY_LIVE_PROXY_BENCH === "1" ? test : test.skip;
const maybeConnectionTest = process.env.AIANDRELAY_LIVE_CONNECTION_BENCH === "1" ? test : test.skip;
const maybeGenerationConnectionTest =
  process.env.AIANDRELAY_LIVE_GENERATION_CONNECTION_BENCH === "1" ? test : test.skip;
const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
const iterations = positiveInt(process.env.AIANDRELAY_LIVE_PROXY_BENCH_ITERATIONS) ?? 5;
const warmup = positiveInt(process.env.AIANDRELAY_LIVE_PROXY_BENCH_WARMUP) ?? 1;
const concurrentRequests = positiveInt(process.env.AIANDRELAY_LIVE_PROXY_BENCH_CONCURRENCY) ?? 3;
const realFetch = globalThis.fetch.bind(globalThis);

afterEach(() => {
  delete process.env.AIANDRELAY_PERF;
  globalThis.fetch = realFetch;
});

maybeTest(
  "live ai& direct vs Claude proxy latency",
  async () => {
    ...
    try {
      ...
      const result = {
        ...
        notes: [
          "Hits the real ai& chat/completions API; this consumes API credits.",
          "Direct rows call ai& directly. Proxy rows call the real Claude proxy handler in-process, which then calls ai&.",
        ],
      };
...
maybeConnectionTest(
  "live ai& connection reuse diagnostic",
  async () => {
    const apiKey = await resolveAiandApiKey();
    const result = {
      endpoint: `${AIAND_BASE_URL}/models`,
      iterations,
      warmup,
      rows: [
      ...
      notes: [
        "Hits ai& /models, not model generation, so it should not consume generation credits.",
...
maybeGenerationConnectionTest(
  "live ai& generation connection reuse diagnostic",
  async () => {
    const apiKey = await resolveAiandApiKey();
      ...
      const result = {
        ...
        sequential: [
          await runPairedSeries(
            ...
            () => withAiandConnectionClose(() => directBuffered(apiKey)),
          ),
          await runPairedSeries(
        ...
        concurrent: [
          await runPairedSeries(
            ...
            () => withAiandConnectionClose(() => runConcurrentBatch(() => directBuffered(apiKey))),
          ),
          await runPairedSeries(
        ...
        proxyCost: costTracker.totals,
        notes: [
          "Hits real ai& chat/completions and consumes API credits.",
          "The forced-close proxy rows wrap benchmark fetch only for ai& upstream URLs, leaving production code unchanged.",
...
async function withAiandConnectionClose<T>(fn: () => Promise<T>): Promise<T> {
  const previousFetch = globalThis.fetch;
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    const href =
      typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
    if (!href.startsWith(AIAND_BASE_URL)) {
...
async function fetchModels(apiKey: string, extraHeaders: Record<string, string>): Promise<number> {
  const started = performance.now();
  const response = await fetch(`${AIAND_BASE_URL}/models`, {
...
async function directBuffered(apiKey: string): Promise<number> {
  const started = performance.now();
  const response = await fetch(`${AIAND_BASE_URL}/chat/completions`, {
    method: "POST",
...
async function resolveAiandApiKey(): Promise<string> {
  const envKey = process.env.AIAND_API_KEY?.trim();
  if (envKey) {
    return envKey;
  }
  const envFile = await readFile(path.join(repoRoot, ".env"), "utf8").catch(() => "");
  const line = envFile.split(/\r?\n/).find((entry) => entry.startsWith("AIAND_API_KEY="));
  const key = line?.slice("AIAND_API_KEY=".length).trim() ?? "";
  if (!key) {
    throw new Error("AIAND_API_KEY is not set and was not found in .env");
  }
  return key;
}
