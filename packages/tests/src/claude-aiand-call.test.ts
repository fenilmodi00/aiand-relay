...
import { fetchAiand } from "@aiandrelay/cli/dist/lib/claude/aiand-call.js";

/**
 * Characterization tests for the ai& HTTP retry loop (#1 prep). These lock
 ...
 * of a shared ai& client (#1) can't silently change it. Uses a mocked
 * global fetch; no network calls.
 *
 * Note: the retry loop sleeps real backoff (1s→2s→4s, from aiand-retry.ts).
...
describe("claude/aiand-call.ts fetchAiand retry contract (#1 characterization)", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  test("200 OK returns the JSON body, no retry", async () => {
    ...
    const result = await fetchAiand({ model: "x" }, { apiKey: "k" }, GLM_5_2);
  ...
  test("401 does NOT retry - non-retryable surfaces immediately", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const result = await fetchAiand({ model: "x" }, { apiKey: "bad" }, GLM_5_2);
