...
describe("Claude context budget utilities", () => {
  test("parses ai& context length token counts from common error shapes", () => {
    const parenthetical =
    ...
    expect(parseAiandContextLengthMaxTokens(parenthetical)).toBe(262144);
    expect(parseAiandContextLengthInputTokens(parenthetical)).toBe(258001);
    expect(parseAiandContextLengthInputTokens(resolved)).toBe(300005);
  });
