...
describe("daemon session persistence", () => {
  test("restores the session-scoped ai& base URL", async () => {
    const home = mkdtempSync(join(tmpdir(), "aiandrelay-daemon-store-"));
    cleanup.push(home);
      ...
      [
        ...
        `
          ...
          store.upsertSession({
            ...
            agent: "claude",
            apiKey: "phantom-key",
            baseUrl: "http://protected-proxy.test/aiand/v1",
    ...
    expect(output).toBe("http://protected-proxy.test/aiand/v1");
  });
});
