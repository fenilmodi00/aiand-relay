...
import { cleanupTmpDir, createTestContext } from "../shared/context.js";
import { startTestDaemon, type TestDaemon } from "./daemon-session.js";
import type { TestContext } from "../shared/types.js";

const TOKEN = "aiandrelay-local-app-registration-test";
...
describe("daemon lazy codex-app session restore", () => {
  ...
  test("re-registers the persisted codex-app session on a token miss instead of 401ing", async () => {
    ...
    // body is on disk (written by `aiandrelay codex-app`) but the daemon
    ...
    // full daemon auth path without needing a real ai& API key.
    ...
    expect(catalog.models?.[0]?.slug).toBe("deepseek-ai/deepseek-v4-flash");
  ...
  test("stops resurrecting the session after restore clears the registration", async () => {
    // `aiandrelay codex-app --restore` deletes both the daemon session and
