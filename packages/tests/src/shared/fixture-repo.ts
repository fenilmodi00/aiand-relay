...
export async function createFixtureRepo(context: TestContext, owner: string): Promise<FixtureRepo> {
  ...
  await writeFile(
    ...
    [
      "# Stats Fixture",
      "",
      "This tiny repo is used by aiandrelay live agent tests.",
      "",
  ...
  await execFileAsync("git", ["commit", "-m", "Initial fixture"], {
    ...
    env: {
      ...process.env,
      GIT_AUTHOR_NAME: "aiandrelay tests",
      GIT_AUTHOR_EMAIL: "tests@aiandrelay.local",
      GIT_COMMITTER_NAME: "aiandrelay tests",
      GIT_COMMITTER_EMAIL: "tests@aiandrelay.local",
    },
  });
