...
describe("runCommand", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(path.join(os.tmpdir(), "aiandrelay-command-"));
    context = {
      repoRoot: tmpDir,
  ...
  test("whoami prints the anonymous install id", async () => {
    ...
    const stored = JSON.parse(await readFile(path.join(home, "install-id"), "utf8")) as {
      id?: string;
    };
