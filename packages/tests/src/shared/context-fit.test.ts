...
import {
  ...
  parseAiandContextLengthMaxTokens,
  stripOldImages,
  trimPayloadMessages,
} from "../../../cli/src/lib/context-fit.js";
import { postChatCompletion, postChatCompletionStream } from "../../../cli/src/lib/aiand-client.js";

const TRIM_MARKER = "[aiandrelay trimmed older context to fit the model window]";
const IMAGE_PLACEHOLDER = "[aiandrelay removed an older image to fit the model window]";
...
describe("contextLengthOverflow", () => {
  ...
  test("parses the vLLM 'in the messages' phrasing ai& returns", () => {
    const vllmMessage =
...
describe("aiand-client context-fit retry", () => {
...
// fails outright instead of self-healing.
describe("Kimi/Moonshot context-overflow phrasing", () => {
  ...
  test("still prefers the standard vLLM phrasing when present", () => {
    ...
    expect(parseAiandContextLengthMaxTokens(msg)).toBe(262144);
    expect(parseAiandContextLengthInputTokens(msg)).toBe(250000);
  });
});
