...
describe("ai& response-header timeout", () => {
  ...
  test("defaults to 45 seconds before rejecting a response-header stall", async () => {
    ...
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS", "");
    vi.stubEnv("AIANDRELAY_FALLBACK_MODEL", "off");
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_RETRIES", "0");
    vi.stubGlobal(
      "fetch",
      vi.fn(
    ...
    await expect(pending).resolves.toMatchObject({
      name: "AiandResponseHeaderTimeoutError",
      timeoutMs: 45_000,
    });
  });

  test("rejects a fetch that never returns headers with a typed, persisted diagnostic", async () => {
    temporaryHome = await mkdtemp(path.join(os.tmpdir(), "aiandrelay-timeout-test-"));
    ...
    expect(error).toBeInstanceOf(AiandResponseHeaderTimeoutError);
    expect(error).toMatchObject({ name: "AiandResponseHeaderTimeoutError", timeoutMs: 100 });
    ...
    expect(persisted.clientRequestId).toBe((error as AiandResponseHeaderTimeoutError).requestId);
    expect(persisted).not.toHaveProperty("apiKey");
  });

  test("preserves a caller abort without retrying it as a transport timeout", async () => {
    temporaryHome = await mkdtemp(path.join(os.tmpdir(), "aiandrelay-abort-test-"));
    vi.stubEnv("AIANDRELAY_HOME", temporaryHome);
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS", "1000");
    ...
    expect(error).not.toBeInstanceOf(AiandResponseHeaderTimeoutError);
  ...
  test("limits non-stream response-header timeouts to one safe retry", async () => {
    temporaryHome = await mkdtemp(path.join(os.tmpdir(), "aiandrelay-buffered-timeout-test-"));
    vi.stubEnv("AIANDRELAY_HOME", temporaryHome);
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS", "100");
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_RETRIES", "1");
    vi.stubEnv("AIANDRELAY_FALLBACK_MODEL", "off");
    const fetchMock = vi.fn(
      (_url: string, init?: RequestInit) =>
        new Promise<Response>((_resolve, reject) => {
