...
export async function assertClaudeContextLimitRetry(context: TestContext): Promise<void> {
  ...
  try {
    ...
    assert(
      stderr.includes("[aiandrelay proxy] context-fit retry") ||
        stderr.includes('"maxTokens":28000') ||
        stderr.includes("retrying aiand request with reduced max_tokens") ||
        ...
        (stderr.includes("aiandrelay: trimmed") && stderr.includes("(retry path")) ||
        (stderr.includes("aiandrelay: DROPPED A LARGE PORTION") && stderr.includes("(retry path")),
...
export async function assertCodexContextLimitRetry(context: TestContext): Promise<void> {
  ...
  try {
    const prompt = [
      "This Responses request intentionally exceeds the Kimi K2.7 Code 262k context window when paired with the requested max_output_tokens.",
    ...
    assert(
      stderr.includes("[aiandrelay proxy] context-fit retry") ||
        ...
        (stderr.includes("aiandrelay: trimmed") && stderr.includes("(retry path")) ||
        (stderr.includes("aiandrelay: DROPPED A LARGE PORTION") && stderr.includes("(retry path")),
      `daemon did not log Codex context-limit retry; stderr=${stderr.slice(-2000)}`,
    );
    assert(
