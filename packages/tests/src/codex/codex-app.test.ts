...
describe("Codex App alpha config", () => {
  test("writes an app-specific provider without dropping existing tables", () => {
    const config = buildCodexAppConfig(
      ...
      ].join("\n"),
      {
        ...
        providerId: "aiandrelay_codex_app",
    ...
    expect(config).toContain('model_provider = "aiandrelay_codex_app"');
    ...
    expect(config).toContain("[model_providers.aiandrelay_codex_app]");
    expect(config).toContain('name = "ai& Relay"');
    ...
    // requests still go to ai& Relay's local base_url.
  ...
  test("replaces an existing managed block instead of appending duplicates", () => {
    ...
    expect(second.match(/>>> aiandrelay codex-app alpha >>>/g)).toHaveLength(1);
    ...
    expect(second).toContain('model_provider = "aiandrelay_codex_app"');
  ...
  test("removes legacy app profile and provider tables", () => {
    const config = buildCodexAppConfig(
      [
        'profile = "aiandrelay_codex_app"',
        "",
        "[profiles.aiandrelay_codex_app]",
        'model = "stale"',
        "",
        "[model_providers.aiandrelay_codex_app]",
    ...
    expect(config).not.toContain('profile = "aiandrelay_codex_app"');
    expect(config.match(/\[profiles\.aiandrelay_codex_app\]/g)).toBeNull();
    expect(config.match(/\[model_providers\.aiandrelay_codex_app\]/g)).toHaveLength(1);
  ...
  test("emits the full ModelInfo schema Codex Desktop expects", () => {
    ...
    // Truncation limit is the context window divided by the Codex↔ai&
    ...
    const vision = catalog.models.find((m) => m.slug === "moonshotai/kimi-k2.7-code");
