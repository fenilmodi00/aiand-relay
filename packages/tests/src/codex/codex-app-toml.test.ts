...
import {
  ...
  tomlString,
} from "@aiandrelay/cli/dist/lib/codex-app/toml.js";

const START = "# >>> aiandrelay codex-app alpha >>>";
const END = "# <<< aiandrelay codex-app alpha <<<";
...
describe("codex-app/toml.ts - pure TOML preamble manipulation (#4)", () => {
  ...
  test("upsertTopLevelTomlKeys updates existing keys and appends new ones", () => {
    ...
    const out = upsertTopLevelTomlKeys(preamble, {
      model: '"new"',
      model_provider: '"aiandrelay"',
    });
    expect(out).toContain('model = "new"');
    expect(out).toContain('model_provider = "aiandrelay"');
  ...
  test("tomlString quotes a string value", () => {
    expect(tomlString("aiandrelay")).toBe('"aiandrelay"');
