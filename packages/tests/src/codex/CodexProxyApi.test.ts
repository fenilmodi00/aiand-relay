...
} from "../../../cli/src/lib/codex/proxy.js";
...
describe("Codex Responses proxy tool compatibility", () => {
  ...
  test("bridges client-executed tool search through ai& function calling", async () => {
  ...
  test("retries streamed Codex turns when ai& never returns response headers", async () => {
    vi.stubEnv("AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS", "100");
  ...
  test("routes chat completions through the session upstream base URL", async () => {
    ...
    const response = await postResponses(
      ...
      { ...options, baseUrl: "http://codex-upstream.test/aiand/v1" },
    );

    expect(response.status).toBe("completed");
    expect(urls).toEqual(["http://codex-upstream.test/aiand/v1/chat/completions"]);
  });
      ...
      vi.fn(async (url: string, init?: RequestInit) => {
        ...
        return jsonResponse({
          choices: [
            {
              message: {
                reasoning_content: "Private ai& reasoning must not enter persisted history.",
  ...
  test("all models compact before ai& tokenizer rejects (1.8x mismatch)", async () => {
    const catalog = await getModels();
  ...
  test("flattens namespace tools for ai& and restores namespace in Codex output", async () => {
    const requests: unknown[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
  ...
  test("retries streamed Codex turns when upstream SSE goes idle before output", async () => {
    const requests: Array<{ body: any }> = [];
    vi.stubEnv("AIANDRELAY_CODEX_STREAM_IDLE_TIMEOUT_MS", "100");
    vi.stubEnv("AIANDRELAY_CODEX_STREAM_IDLE_RETRIES", "1");
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
        if (url.startsWith("http://127.0.0.1:")) {
  ...
  test("retries transient ai& rate limits before returning a Codex response", async () => {
    const requests: unknown[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
  ...
  test("strips native web_search from upstream Responses tools", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init?: RequestInit) => {
        ...
        return jsonResponse({
          choices: [{ message: { content: "Answered without native search." } }],
        });
      }),
    );
    ...
    const upstream = requests.find((request) => request.url.includes("api.aiand.com"));
    expect(upstream?.body.tools).toBeUndefined();
    expect(requests.filter((request) => request.url.includes("api.aiand.com"))).toHaveLength(1);
    expect(response.output[0]).toMatchObject({
      type: "message",
      role: "assistant",
      content: [
        {
          type: "output_text",
          text: "Answered without native search.",
          annotations: [],
        },
      ],
    });
  ...
  test("strips native web_search from streamed upstream Responses tools", async () => {
    const requests: Array<{ url: string; body: any }> = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
        ...
        requests.push({ url, body });
        return sseResponse([
          { choices: [{ delta: { content: "No native search. " } }] },
    ...
    expect(response).toContain("No native search.");
    expect(response).toContain("response.completed");
    expect(requests.filter((request) => request.url.includes("api.aiand.com"))).toHaveLength(1);
    expect(requests[0]?.body.tools).toBeUndefined();
  });

  test("fails streamed completion when upstream SSE goes idle", async () => {
    const requests: Array<{ url: string; body: any }> = [];
    vi.stubEnv("AIANDRELAY_CODEX_STREAM_IDLE_TIMEOUT_MS", "100");
    vi.stubEnv("AIANDRELAY_CODEX_STREAM_IDLE_RETRIES", "1");
    vi.stubGlobal(
    ...
    expect(response).toContain("response.failed");
    expect(response).toContain("ai& stream produced no SSE event for 100ms.");
    expect(response).not.toContain("response.completed");
    expect(requests.filter((request) => request.url.includes("api.aiand.com"))).toHaveLength(2);
    expect(requests[0]?.body.stream).toBe(true);
  });
  ...
  test("fails when stream emits reasoning but never final output", async () => {
    const requests: Array<{ url: string; body: any }> = [];
    vi.stubEnv("AIANDRELAY_CODEX_STREAM_TURN_TIMEOUT_MS", "100");
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
        if (url.startsWith("http://127.0.0.1:")) {
    ...
    expect(response).toContain("response.failed");
    expect(response).toContain("ai& stream exceeded maximum turn duration of 100ms.");
  ...
  test("does not execute custom function web_search when grouped with client tools", async () => {
    const requests: Array<{ url: string; body: any }> = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
    ...
    expect(requests.filter((request) => request.url.includes("api.aiand.com"))).toHaveLength(1);
    expect(requests.some((request) => request.url.includes("api.tavily.com"))).toBe(false);
    expect(response.output).toEqual([
      {
        id: expect.stringMatching(/^fc_/),
        type: "function_call",
  ...
  test("forwards Responses input_image parts to ai& vision message content", async () => {
    const requests: unknown[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
  ...
  test("routes Desktop per-turn vision model selections instead of the session default", async () => {
    ...
    expect(upstream.model).toBe(QWEN_3_6_27B.id);
    expect(upstream.messages.at(-1)).toEqual({
      role: "user",
      content: [
        { type: "text", text: "Describe this." },
  ...
  test("translates forced namespace tool_choice to the flattened ai& tool name", async () => {
    const requests: unknown[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
  ...
  test("maps ai& reasoning token usage into Responses usage details", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
        if (url.startsWith("http://127.0.0.1:")) {
  ...
  test("defaults max_tokens to the model output budget when Codex omits max_output_tokens", async () => {
    ...
    // ai& silently caps omitted max_tokens at 2048, which truncates
    ...
    expect(requests[0]?.max_completion_tokens).toBe(GLM_5_2.limit.output);
    ...
    expect(requests[1]?.max_completion_tokens).toBe(1234);
  });

  test("routes Codex memory extraction requests to the default long-context ai& model", async () => {
    const requests: unknown[] = [];
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  rollout_summary: "Captured ai& Relay memory support investigation.",
                  rollout_slug: "aiandrelay_codex_memory_support",
                  raw_memory:
                    "ai& Relay should route Codex memory extraction separately from the main coding model.",
                }),
              },
            },
          ],
    ...
    expect(upstream.model).toBe(MOTIF_3.id);
  ...
  test("allows Codex memory extraction model override from env", async () => {
    const requests: unknown[] = [];
    vi.stubEnv("AIANDRELAY_CODEX_MEMORY_MODEL", GEMMA_4_31B.id);
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: string, init?: RequestInit) => {
        if (url.startsWith("http://127.0.0.1:")) {
    ...
    expect(upstream.model).toBe(GEMMA_4_31B.id);
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init?: RequestInit) => {
        ...
        if (togetherCallCount === 1) {
          // Simulate an ai& template-error 400 (e.g. a `keys` collision
    ...
    const response = await postResponsesText({
      ...
      input: [
        { type: "message", role: "user", content: [{ type: "input_text", text: "Go." }] },
        {
          ...
          // (only `items` is proactively renamed), so it reaches ai&
  ...
  test("trims old context and retries when input alone exceeds the context window", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init?: RequestInit) => {
        ...
        callCount += 1;
        if (callCount === 1) {
          // ai& rejects: input alone (325k) exceeds the 262k window.
          return new Response(
            JSON.stringify({
              error: {
                message:
    ...
    const hasTrimMarker = retryMessages.some(
      (m: any) => typeof m.content === "string" && m.content.includes("[aiandrelay trimmed"),
    );
