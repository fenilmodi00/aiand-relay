...
export type TestDaemon = {
  url: string;
  /** The daemon's isolated AIANDRELAY_HOME. */
  home: string;
  stderr: () => string;
  stop: () => Promise<void>;
};
...
async function registerSession(
  ...
  const apiKey = await resolveAiandApiKey(context.repoRoot);
...
async function resolveAiandApiKey(repoRoot: string): Promise<string> {
  ...
  assert(key.length > 0, "AIAND_API_KEY is not set and was not found in .env");
  return key;
}
