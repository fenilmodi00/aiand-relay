...
/**
 * `--no-mcp` is a aiandrelay convenience: Codex connects to every MCP server
...
export async function runCodexAiand(options: CodexLaunchOptions): Promise<CodexLaunchResult> {
  ...
  const result: ProxiedSessionResult = await runProxiedSession({
    ...
    banner: (modelName) => `ai& Relay ▸ Routing Codex → ai& (${modelName}). Not OpenAI.\n`,
    beforeSpawn: () => {
...
function codexConfigArgs(
  ...
  return [
    ...
    "-c",
    `model_providers.${CODEX_PROVIDER_ID}.name="ai& Relay"`,
    "-c",
...
function writeCodexModelCatalog(): { path: string; cleanup: () => void } {
  const dir = mkdtempSync(join(tmpdir(), "aiandrelay-codex-catalog-"));
