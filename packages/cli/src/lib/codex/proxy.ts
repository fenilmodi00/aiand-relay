...
import { streamResponseFromAiand } from "./stream.js";
import type { ResponsesRequest, ResponsesTool } from "./wire-types.js";

export type CodexProxyOptions = {
  apiKey: string;
  /** Session-scoped ai& API root resolved by the launching process. */
  baseUrl: string;
  modelId: string;
...
export async function handleCodexProxyRequest(
  ...
  const translated = perf.spanSync("translate_request", () => {
    ...
    return { toolTranslation, requestModel, translatedPayload };
  });
  const { toolTranslation, requestModel, translatedPayload } = translated;
  ...
  const chatResponse = await perf.span("upstream_fetch", () =>
    callAiandChat(translatedPayload, options, requestModel.definition, upstreamAbort.signal),
  );
