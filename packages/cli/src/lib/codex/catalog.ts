...
// Safety margin for the Codex ↔ ai& tokenizer mismatch.
//
// Codex counts tokens with its own tokenizer, which consistently
// underestimates relative to ai&'s server-side count: the mismatch is
// ~1.77× for text+tool-schemas and even higher with vision content. With the
// full context window Codex never compacts until ai& already rejected
...
// truncates *proactively*, before ai&'s tokenizer rejects:
...
//
// The proxy's reactive input-trim (aiand-call.ts) remains the backstop
...
export function toCodexModelCatalogEntry(
  ...
  return {
    ...
    description: `ai& model via aiandrelay (${model.definition.id})`,
    ...
    // Codex's catalog schema only accepts text|image|audio (rejects pdf/video).
    input_modalities: model.definition.modalities.input.filter(
      (m) => m === "text" || m === "image" || m === "audio",
    ),
