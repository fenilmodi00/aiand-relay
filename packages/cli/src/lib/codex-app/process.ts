...
export function codexAppLaunchMessage(result: CodexAppLaunchResult): string {
  ...
  if (result.wasRunning && result.restartUnsupported) {
    return "ChatGPT App is already open, but aiandrelay could not restart it. Quit and reopen ChatGPT App when you are ready.";
  }
...
async function shouldRestartCodexApp(reason: CodexAppLaunchReason): Promise<boolean> {
  ...
  const action =
    reason === "restored" ? "reload your restored ChatGPT profile" : "reload the ai& Relay profile";
...
async function spawnDetached(command: string, args: string[]): Promise<boolean> {
  return new Promise((resolve) => {
    const child = spawnBinary(command, args, {
      detached: true,
      stdio: "ignore",
    });
