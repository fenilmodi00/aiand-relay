...
export function appSessionLockPath(home: string): string {
  return path.join(aiandrelayHomeDir(home), "codex-app", "session.json");
}
...
export async function assertNoLiveCodexAppSession(home: string): Promise<void> {
  ...
  throw new Error(
    `Another aiandrelay chatgpt session appears to be running (pid ${lock.pid}). Stop it with Ctrl+C, or run \`aiandrelay chatgpt --restore\` after it exits.`,
  );
}

/**
 * Is the codex config at ~/.codex/config.toml one that aiandrelay wrote?
...
// avoid a dead-import lint once aiandrelayHomeDir is consolidated (see #7).
