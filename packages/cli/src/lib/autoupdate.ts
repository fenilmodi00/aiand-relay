 ...
 * `<home>/.aiandrelay/bin/aiandrelay.js`, launched by a tiny `aiandrelay`
...
import { HARNESS_WRAPPER_COMMANDS } from "./cli/wrappers.js";
...
/**
 * Where the install lives. `AIANDRELAY_HOME` (when set) is the `.aiandrelay`
 * directory itself - matching `scripts/install.sh`, which installs the bundle
 * at `$AIANDRELAY_HOME/bin/aiandrelay.js`. When unset, default to
 * `~/.aiandrelay`.
 */
function resolveInstallDir(): string {
  return process.env.AIANDRELAY_HOME || path.join(os.homedir(), ".aiandrelay");
}

/** Installed bundle path. `aiandrelay` wrapper runs `bun run` on this. */
function installedBundlePath(): string {
  return path.join(resolveInstallDir(), "bin", "aiandrelay.js");
}
...
async function fetchManifest(): Promise<Manifest> {
  const res = await withTimeout(
    fetch(resolveManifestUrl(), {
      headers: { "User-Agent": `aiandrelay/${VERSION}` },
 ...
 *
 * Auto-update only replaces the bundle, so a release that ADDS a wrapper
 * (e.g. `aprime`) would never reach existing installs. Back these in after
 * an update so new harnesses become usable immediately.
 */
export async function backfillWrapperCommands(): Promise<string[]> {
  ...
  for (const { name, harness } of HARNESS_WRAPPER_COMMANDS) {
...
export async function maybeSelfUpdate(): Promise<void> {
  ...
  // AIANDRELAY_DISABLE_AUTOUPDATE=1 to opt out.
  if (process.env.AIANDRELAY_DISABLE_AUTOUPDATE === "1") {
    return;
  }
  if (!isInstalledBundle()) {
