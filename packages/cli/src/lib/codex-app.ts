...
const CODEX_APP_CONFIG_MARKER_START = "# >>> aiandrelay codex-app alpha >>>";
const CODEX_APP_CONFIG_MARKER_END = "# <<< aiandrelay codex-app alpha <<<";
/**
 * ChatGPT Desktop gates the model picker on provider "OpenAI auth" state for
 * custom providers (openai/codex#10867). We set `requires_openai_auth = true`
 * so the picker unlocks; traffic still goes to our local `base_url`.
 */
const CHATGPT_DESKTOP_REQUIRES_OPENAI_AUTH = true;
...
export async function runCodexAppCommand(ctx: HarnessContext): Promise<HarnessResult> {
  ...
  // what ai& serves. Best-effort; falls back to the bundled snapshot.
  ...
  if (!apiKey) {
    throw new Error(
      "No ai& API key found. Pass --api-key, run `aiandrelay configure`, or set AIAND_API_KEY.",
    );
  }
  ...
  await writeAppRegistration(registration, aiandrelayHomeDir(ctx.home));
  ...
  void sendTelemetryEvent({
    ...
    metadata: {
      ...
      providerId: CODEX_APP_PROVIDER_ID,
      providerAuthWorkaround: CHATGPT_DESKTOP_REQUIRES_OPENAI_AUTH,
  ...
  const intro = [
    "ChatGPT App profile changed to ai& Relay. (alpha)",
    `Model: ${selectedModel.definition.name}`,
    "Start a task or open a repository in ChatGPT App as usual.",
    "Restore your previous ChatGPT App profile with: aiandrelay chatgpt --restore",
    `Backup: ${backup}`,
...
export function buildCodexAppConfig(
  ...
  const cleanedPreamble = removeTopLevelTomlKeys(managedPreamble, [
    ...
    // versions of the aiandrelay managed config. They become stale the
  ...
  const providerBlock = [
    CODEX_APP_CONFIG_MARKER_START,
    "# aiandrelay codex-app configures a dedicated alpha provider for ChatGPT Desktop.",
    `[model_providers.${options.providerId}]`,
    ...
    "# actual model requests still go to the local ai& Relay base_url above.",
    "# See https://github.com/openai/codex/issues/10867",
    `requires_openai_auth = ${CHATGPT_DESKTOP_REQUIRES_OPENAI_AUTH ? "true" : "false"}`,
    CODEX_APP_CONFIG_MARKER_END,
    "",
  ].join("\n");
...
async function restoreCodexApp(home: string): Promise<HarnessResult> {
  ...
  await clearAppRegistration(aiandrelayHomeDir(home));
  // Restore should also drop the models cache: a stale OpenAI-only cache left
  // behind by a aiandrelay session would make Codex show "Unknown model"
...
function modelCatalogPath(home: string): string {
  return path.join(home, ".codex", "aiandrelay-codex-app-models.json");
}
...
function aiandrelayHomeDir(home: string): string {
  return process.env.AIANDRELAY_HOME || path.join(home, ".aiandrelay");
}
