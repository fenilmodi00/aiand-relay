...
export type AiandRequestDiagnostic = {
  at: string;
...
export async function persistRequestDiagnostic(
  diagnostic: Omit<AiandRequestDiagnostic, "at">,
): Promise<void> {
  if (process.env.AIANDRELAY_REQUEST_DIAGNOSTICS === "0") {
    return;
  }
...
export function resolveRequestDiagnosticsPath(home = aiandrelayHome()): string {
