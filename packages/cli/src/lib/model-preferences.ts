...
// aiandrelayHome() is the one home both the daemon and the launcher resolve
// (via AIANDRELAY_HOME or ~/.aiandrelay), so the daemon's writes and the
// launcher's reads always hit the same file.
function preferencesPath(): string {
  return path.join(aiandrelayHome(), "preferences.json");
}
