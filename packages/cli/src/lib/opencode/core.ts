...
import { AIAND_API_KEY_ENV_REF } from "../aiand-core.js";
import {
...
type OpencodeConfig = {
   ...
   * namespace, not `zen/*`, per opencode issue #6979). aiandrelay routes
   * everything to ai&, so Zen's auto-loaded models are pure clutter in
   * the picker; this keeps /models to only the ai& flagships we curate.
   ...
   * is no config field for that - but it means only `aiand` is active, so
...
type OpencodeProviderConfig = {
   ...
   * (otherwise OpenCode merges our declared models on top of ai&'s full
 ...
 * self-branding, and a `vision` subagent is added on a vision-capable ai&
...
export function buildOpencodeConfigJson({
  ...
  apiKeyEnvRef = AIAND_API_KEY_ENV_REF,
  ...
  // Register every model in the live ai& catalog (the full set /models
  ...
  const provider: OpencodeProviderConfig = {
    // ai& has no first-party AI SDK adapter, so use the generic
    // OpenAI-compatible provider and point it at ai&'s base URL explicitly.
    ...
    // (opencode #20968).
    name: "ai&",
    options: { apiKey: apiKeyEnvRef, baseURL: AIAND_BASE_URL },
    models,
    // Restrict /models to exactly the curated set. Without this, OpenCode also
    // shows ai&'s full catalog (hundreds of models) because the `models`
  ...
  return {
    ...
    // explicitly pins a vision-capable ai& model (Kimi-K2.7-Code) so a
    ...
    // Only load our ai& provider; ignore every other provider (Anthropic,
 ...
 * precedence, never persisted) plus the resolved ai& key so
 * `{env:AIAND_API_KEY}` substitution resolves inside the config.
 */
