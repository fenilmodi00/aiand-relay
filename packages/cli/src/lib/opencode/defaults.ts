...
export const OPENCODE_PROVIDER_ID = "aiand";

// Real ai& model id. OpenCode-facing selector is
 ...
 * `/models` shows (ai&'s full serverless catalog is hidden via the
...
/**
 * Whitelist of model ids that restricts the ai& provider so `/models` shows
 * ONLY our live catalog set (opencode PR #3416). Without this, OpenCode merges
 * our declared models on top of ai&'s full models.dev catalog, surfacing
...
/** OpenCode selector form for the vision subagent's model: provider/<aiand-id>. */
 ...
 * the assistant doesn't self-identify as OpenCode when running on ai&.
