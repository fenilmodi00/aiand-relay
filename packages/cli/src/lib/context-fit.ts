...
/**
 * Shared, wire-uniform reactive context-fit for the ai& client.
 *
 * Every proxied request (Claude + Codex, stream + non-stream) funnels through
 * `aiand-client.ts` as an OpenAI chat-completions payload. When ai&
 ...
 * ai&'s *actual* reported input-token count each time (not an estimate),
...
const IMAGE_REMOVED_PLACEHOLDER = "[aiandrelay removed an older image to fit the model window]";
const TRIM_MARKER = "\n[aiandrelay trimmed older context to fit the model window]\n";
...
// --------------------------------------------------------------------------
// Parsing ai&'s context-length error
// --------------------------------------------------------------------------
...
export function parseAiandContextLengthInputTokens(message: string): number | undefined {
  ...
  // vLLM (which ai& runs) phrases the overflow differently:
  ...
  // generic "input tokens" fallback so the reactive context-fit fires on ai&.
...
/**
 * Recognize an ai& context-length rejection and extract the real input and
...
export function contextLengthOverflow(
  ...
  const inputTokens = parseAiandContextLengthInputTokens(message);
  if (inputTokens === undefined) {
    return undefined;
  }
  const contextTokens = parseAiandContextLengthMaxTokens(message) ?? model.limit.context;
 ...
 * vision-expanded by ai& into far more tokens than their JSON bytes
 ...
 * that's fine, the client re-reads ai&'s true count on the next attempt.
 */
...
export function newContextFitState(payload: Record<string, unknown>): ContextFitState {
  const maxField = readMaxOutputTokens(payload);
  const originalMaxTokens =
    maxField !== undefined ? Math.max(CONTEXT_LENGTH_RETRY_FLOOR, Math.floor(maxField)) : undefined;
  return {
...
function readMaxOutputTokens(payload: Record<string, unknown>): number | undefined {
  if (
...
function writeMaxOutputTokens(payload: Record<string, unknown>, value: number): void {
  if ("max_completion_tokens" in payload || !("max_tokens" in payload)) {
 ...
 * again with ai&'s fresh count, so the ladder escalates across attempts:
...
export function applyContextFit(
  ...
  // requested output. Clamp max output tokens and keep every token of context.
  const availableOutput = contextTokens - inputTokens - CONTEXT_OUTPUT_SAFETY_TOKENS;
  const currentMaxTokens = readMaxOutputTokens(payload);
  ...
  if (availableOutput >= minPreferredOutput) {
    ...
    if (currentMaxTokens === undefined || nextMaxTokens < currentMaxTokens) {
      writeMaxOutputTokens(payload, nextMaxTokens);
  ...
  if (currentMaxTokens !== desiredMaxTokens) {
    writeMaxOutputTokens(payload, desiredMaxTokens);
  }
...
export function emitContextTrimAlarm(info: ContextTrimTelemetryInfo): void {
  ...
  process.stderr.write(
    `aiandrelay: ${severity} ${info.trimmedChars} chars of conversation context ` +
