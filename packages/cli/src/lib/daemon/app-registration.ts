 ...
 *
 * `aiandrelay codex-app` configures the Codex desktop app once and exits, so
 ...
 * `aiandrelay codex-app`. Persisting the full register body lets the daemon
 * rebuild the session on demand instead.
 */
export function appRegistrationPath(home = aiandrelayHome()): string {
...
export async function writeAppRegistration(
  ...
  // 0600: the body carries the real ai& API key, like daemon.sqlite.
...
export async function clearAppRegistration(home = aiandrelayHome()): Promise<void> {
 ...
 * next `aiandrelay codex-app` run rewrites it.
 */
