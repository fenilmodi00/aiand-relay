...
export async function createSessionStore(home = aiandrelayHome()): Promise<SessionStore> {
...
export function resolveSessionDatabasePath(home = aiandrelayHome()): string {
...
function warnStoreError(action: string, err: unknown): void {
  ...
  process.stderr.write(`[aiandrelay daemon] Could not ${action}: ${message}\n`);
}
