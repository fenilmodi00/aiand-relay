 ...
 *   speaks Anthropic shape; the daemon translates to ai&'s OpenAI shape),
 *   so the daemon owns the `CostTracker` and accounts tokens as they flow.
 * - `opencode`: the agent runs DIRECT to ai& (no proxy - the `@ai-sdk/
 *   openai-compatible adapter is pointed at ai&, and OpenCode handles images
 ...
 *   translates it to ai& chat completions.
...
export type SessionState = {
  ...
  modelLabel: string;
  /** Real ai& API key the daemon uses upstream (proxied) or that the
    ...
    for (const session of persisted) {
      if (session.pid !== undefined && !isProcessAlive(session.pid)) {
        this.store.markSessionEnded(
          session.token,
          now,
          "[aiandrelay cost] session total: $0.0000 (0 in, 0 out)",
      ...
      if (session.pid === undefined && isNoPidSessionIdle(lastSeenAt, now)) {
        this.store.markSessionEnded(
          session.token,
          now,
          session.externalSummary ?? "[aiandrelay cost] session total: $0.0000 (0 in, 0 out)",
          {
...
export function buildSession(req: RegisterSessionRequest): SessionState {
  ...
  const now = Date.now();
  const baseUrl = req.baseUrl ?? AIAND_BASE_URL;
