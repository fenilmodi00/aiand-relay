 ...
 * `AIANDRELAY_HOME` (matching autoupdate.ts/install.sh's install dir) so a
 * user with a custom install home keeps the pid file alongside the bundle.
 */
export function daemonPidPath(home = aiandrelayHome()): string {
  return path.join(home, "daemon.pid");
}

export function resolveDaemonPort(): number {
  const raw = process.env.AIANDRELAY_PORT;
      ...
      if (err.code === "EADDRINUSE") {
        ...
        void probeHealthz(port).then((healthy) => {
          ...
          process.stderr.write(
            `[aiandrelay daemon] port ${port} in use by a non-daemon process.\n`,
          );
          process.exit(1);
        });
        return;
 ...
 * Anthropic errors because the catch-all imported `isAiandApiError` +
 ...
 * `aiandrelay claude` session: each registers its token + credentials at
...
export async function runDaemon(options: DaemonOptions = {}): Promise<void> {
  ...
  const debug = options.debug ?? process.env.AIANDRELAY_DEBUG === "1";
  ...
  // Refresh the live ai& model catalog (modality, pricing, context) in the
  ...
  void initModelCatalog({ home: os.homedir() }).then(() => {
    if (debug) {
      process.stderr.write(`[aiandrelay daemon] model catalog loaded.\n`);
    }
  });
  ...
  if (debug) {
    process.stderr.write(
      `[aiandrelay daemon] listening: ${daemonUrl(port)} (pid ${process.pid})\n`,
    );
    if (restored > 0) {
      process.stderr.write(`[aiandrelay daemon] restored ${restored} active session(s).\n`);
    }
  }
  ...
  const reaper = setInterval(() => {
    ...
    if (debug && removed > 0) {
      process.stderr.write(`[aiandrelay daemon] reaped ${removed} dead session(s).\n`);
    }
  ...
  const shutdown = async (signal: NodeJS.Signals) => {
    ...
    if (debug) {
      process.stderr.write(`[aiandrelay daemon] ${signal} - shutting down.\n`);
    }
...
async function handleDaemonRequest(
  ...
  if (opts.debug) {
    process.stderr.write(`[aiandrelay daemon] ${req.method} ${path_}\n`);
  }
  ...
  // secret either). Used only by `aiandrelay` itself.
  if (path_ === "/internal/sessions") {
    ...
    if (req.method === "GET") {
      ...
      // (it authorizes billing against that session's ai& apiKey), so
  ...
  if (usageMatch && req.method === "POST") {
    ...
    // Self-reported cost (e.g. OpenCode at exit, which goes direct to ai&
