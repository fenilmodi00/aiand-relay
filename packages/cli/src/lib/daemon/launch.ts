...
import { aiandrelayHome, isProcessAlive } from "../paths.js";
...
export async function ensureDaemon(): Promise<{ url: string }> {
  ...
  // the entry the user invoked (the bundle path, or `dist/bin/aiandrelay.js`
  ...
  throw new Error(
    `aiandrelay daemon did not become healthy on ${url} within ${HEALTH_POLL_TIMEOUT_MS / 1000}s. ` +
      `Set AIANDRELAY_PORT to use a different port.`,
  );
}
...
function daemonMatchesCurrentScript(health: DaemonHealth, current: ScriptIdentity): boolean {
  if (health.home !== null && health.home !== aiandrelayHome()) {
    return false;
  }
 ...
 * under the install, or `dist/bin/aiandrelay.js` under `pnpm dev`), resolved
...
export async function localProxyAuthToken(): Promise<string> {
  const file = path.join(aiandrelayHome(), LOCAL_PROXY_TOKEN_FILE);
  try {
  ...
  const token = `aiandrelay-local-${randomBytes(32).toString("base64url")}`;
...
export function startDaemonSessionKeepalive(
  ...
  const recover = async (reason: string) => {
    ...
    if (options.debug) {
      process.stderr.write(
        `[aiandrelay daemon] restored ${options.label ?? registration.agent ?? "session"} after ${reason}.\n`,
      );
    }
  };
  ...
  const safeRecover = async (reason: string) => {
    try {
      ...
      if (options.debug) {
        process.stderr.write(
          `[aiandrelay daemon] could not restore ${options.label ?? registration.agent ?? "session"}: ${
