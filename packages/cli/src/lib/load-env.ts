...
const LOADABLE_ENV_KEYS = new Set(["AIAND_API_KEY"]);
 ...
 * Only the ai& key is loaded from project .env files.
 *
 * Looks first in the directory the CLI was invoked from (cwd), then walks up
 * to the repo root so `aiandrelay claude` run from a workspace picks up the
 * shared root .env.
 */
