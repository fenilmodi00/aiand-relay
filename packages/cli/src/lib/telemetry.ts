...
import { readJsonIfExists, writeJsonAtomic } from "./aiand-core.js";
import { aiandrelayHome } from "./paths.js";
import { VERSION } from "./version.js";

// Telemetry is off by default. Send only when AIANDRELAY_TELEMETRY_URL is set
// (and not disabled). Resolved at call time so tests can override.
function telemetryEndpoint(): string | undefined {
  return process.env.AIANDRELAY_TELEMETRY_URL;
}
...
export type ContextTrimTelemetryInfo = {
  ...
  /** Model id the input was trimmed to fit (ai& target model id). */
  model: string;
...
function installIdPath(): string {
  return path.join(aiandrelayHome(), "install-id");
}

const pendingInstallIds = new Map<string, Promise<string>>();

function telemetryDisabledByEnvironment(): boolean {
  return process.env.GITHUB_ACTIONS === "true" || process.env.AIANDRELAY_DISABLE_TELEMETRY === "1";
}
