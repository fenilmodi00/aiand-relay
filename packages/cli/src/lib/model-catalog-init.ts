...
import { applyCatalog, buildCatalog, AIAND_BASE_URL, type AiandApiModel } from "@aiandrelay/models";
import { aiandrelayHome } from "./global-config.js";
import { readJsonIfExists, resolveAiandApiKey, writeJsonAtomic } from "./aiand-core.js";

/**
 * Load the live ai& model catalog and install it as the active one.
 *
 * The catalog (which models exist and, crucially, each model's modality) comes
 * from `GET /v1/models` so it always matches what ai& serves - no
 * hand-maintained list. Results are cached to `~/.aiandrelay/
...
type CatalogCache = {
  fetchedAt: number;
  baseUrl: string;
  models: AiandApiModel[];
};

function cachePath(home?: string): string {
  return path.join(aiandrelayHome(home), "model-catalog.json");
}
...
async function loadCatalog(options: InitModelCatalogOptions): Promise<void> {
  ...
  const baseUrl = (options.baseUrl ?? AIAND_BASE_URL).replace(/\/$/, "");
...
async function fetchModels(apiKey: string, baseUrl: string): Promise<AiandApiModel[]> {
  ...
  try {
    ...
    const body = (await res.json()) as { data?: AiandApiModel[] };
