...
import { backoffMs, parseRetryAfter, sleep } from "./aiand-retry.js";
...
import { applyAiandChatWire } from "./chat-wire.js";

/**
 * The deep ai& HTTP client - one place for the POST /chat/completions
 * retry loop that used to be copy-pasted three times (claude/aiand-call.ts,
 * claude/stream.ts:postAiandStream, codex/aiand-call.ts). Carved out so
 * `aiand-core.ts`'s name finally earns its keep: the retry contract
 ...
 * *reactive context-fit* retry: when ai& rejects a request with
...
// Transient upstream faults: 429/503 use the full retry budget. 502/504 get at
// most one retry (SPEC §12). 402 and other 4xx never retry.
const RETRYABLE_STATUSES = new Set([429, 503]);
const GATEWAY_RETRY_STATUSES = new Set([502, 504]);

export const MAX_RETRIES = 3;
const DEFAULT_STREAM_RETRIES = 1;
const MAX_GATEWAY_RETRIES = 1;
// Time to wait for ai& to return the first response headers before failing
// over. 45s catches genuinely-slow-but-working models (large prefill, light
// queuing) while failing over reasonably fast when a model's ai& endpoint is
...
// Override with AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS.
...
// headers (its ai& endpoint is down/overloaded), the relay transparently
...
// pays the timeout. Configure with AIANDRELAY_FALLBACK_MODEL (set to
// "off"/"none" to disable) and AIANDRELAY_FALLBACK_COOLDOWN_MS.
const DEFAULT_FALLBACK_MODEL = "motif-technologies/motif-3";
...
function fallbackModel(): string | undefined {
  const raw = process.env.AIANDRELAY_FALLBACK_MODEL;
  if (raw === undefined) {
...
function fallbackCooldownMs(): number {
  const raw = Number.parseInt(process.env.AIANDRELAY_FALLBACK_COOLDOWN_MS ?? "", 10);
...
export type AiandResponseDiagnostics = {
  clientRequestId: string;
  upstreamRequestId?: string | undefined;
};

const responseDiagnostics = new WeakMap<Response, AiandResponseDiagnostics>();

export class AiandResponseHeaderTimeoutError extends Error {
  constructor(
    readonly timeoutMs: number,
    readonly requestId: string,
  ) {
    super(
      `ai& returned no response headers within ${timeoutMs}ms ` +
        `(client request ID: ${requestId}).`,
    );
    this.name = "AiandResponseHeaderTimeoutError";
  }
}

export function getAiandResponseDiagnostics(
  response: Response,
): AiandResponseDiagnostics | undefined {
  return responseDiagnostics.get(response);
}

export type AiandClientOptions = {
  apiKey: string;
...
export async function postChatCompletion(
  ...
  options: AiandClientOptions,
  signal?: AbortSignal,
  fit?: ContextFitConfig,
): Promise<Response> {
  const wired = applyAiandChatWire(payload, {
    ...(fit?.modelDefinition ? { modelDefinition: fit.modelDefinition } : {}),
  });
  ...
  if (!fit) {
    return doFetch(JSON.stringify(wired));
  }
  return fetchWithContextFit(wired, fit, doFetch);
}

/** One transient-retry attempt-set against ai&, sending `body` verbatim. */
async function postChatCompletionOnce(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
    ...
    try {
      response = await fetchAiandResponse(body, options, signal, attempt);
    } catch (err) {
      ...
      if (err instanceof AiandResponseHeaderTimeoutError) {
  ...
  return syntheticOverloadedResponse("ai& request failed after retries.");
}
 ...
 *
 * `body` lets callers that have already serialized the payload resend identical
 * bytes; in that mode the context-fit retry is skipped (the caller owns
 * (re)serialization).
 */
export async function postChatCompletionStream(
  ...
  if (body !== undefined || !fit) {
    return doFetch(body ?? JSON.stringify(wired));
  }
  return fetchWithContextFit(wired, fit, doFetch);
}
...
async function streamFetchOnce(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      ...
      const allowedRetries =
        err instanceof AiandResponseHeaderTimeoutError ? responseHeaderRetries() : streamRetries();
  ...
  throw new Error("ai& stream request failed after retries.");
}

async function fetchAiandResponse(
  body: string,
  options: AiandClientOptions,
  ...
  let timeoutError: AiandResponseHeaderTimeoutError | undefined;
  ...
  const timeout = setTimeout(() => {
    timeoutError = new AiandResponseHeaderTimeoutError(timeoutMs, clientRequestId);
  ...
  try {
    ...
    if (process.env.AIANDRELAY_UPSTREAM_METRICS === "1") {
      headers["X-Aiand-Metrics"] = "true";
    }
    const response = await fetch(`${options.baseUrl ?? AIAND_BASE_URL}/chat/completions`, {
      method: "POST",
      headers,
      body,
...
function responseHeaderTimeoutMs(): number {
  const raw = process.env.AIANDRELAY_RESPONSE_HEADER_TIMEOUT_MS;
...
function streamRetries(): number {
  const raw = process.env.AIANDRELAY_STREAM_RETRIES;
  const parsed = raw ? Number.parseInt(raw, 10) : NaN;
...
function responseHeaderRetries(): number {
  const raw =
    process.env.AIANDRELAY_RESPONSE_HEADER_RETRIES ?? process.env.AIANDRELAY_STREAM_RETRIES;
  const parsed = raw ? Number.parseInt(raw, 10) : NaN;
  return Number.isFinite(parsed) && parsed >= 0 ? Math.floor(parsed) : DEFAULT_STREAM_RETRIES;
}
...
/**
 * The shared reactive context-fit loop. Posts, and while ai& returns a
 ...
 * ai&'s real reported token count each time so it converges.
 *
 * Invariants:
  ...
  for (let attempt = 0; attempt < CONTEXT_FIT_MAX_ATTEMPTS; attempt += 1) {
    ...
    if (fit.debug) {
      process.stderr.write(
        `[aiandrelay proxy] context-fit retry (${outcome.action}): ` +
