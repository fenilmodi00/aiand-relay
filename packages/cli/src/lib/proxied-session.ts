 ...
 * `runClaudeAiand` and `runCodexAiand`. Those two were ~90% identical
...
export type ProxiedSessionSpec = {
  ...
  agent: "claude" | "codex";
  apiKey: string;
  /** Upstream ai& API root, resolved once by the launching CLI process. */
  baseUrl: string;
  /** Resolved model: the id ai& expects + the human name for the banner. */
  modelId: string;
  ...
  binary: string;
  /** Banner line written to stderr so the user knows this routes to ai&. */
...
export async function runProxiedSession(spec: ProxiedSessionSpec): Promise<ProxiedSessionResult> {
  const debug = process.env.AIANDRELAY_DEBUG === "1";
  ...
  try {
    ...
    throw new Error(
      `Could not register this ${spec.agent === "claude" ? "Claude" : "Codex"} session with the aiandrelay daemon: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  ...
  if (debug) {
    process.stderr.write(`[aiandrelay proxy] daemon: ${proxyUrl}\n`);
    process.stderr.write(`[aiandrelay proxy] session: ${agentProxyUrl}\n`);
    process.stderr.write(`[aiandrelay ${spec.agent}] model: ${spec.modelId}\n`);
  }
  ...
  const result = await new Promise<ProxiedSessionResult>((resolve) => {
    child.on("error", (err) => {
      process.stderr.write(`ai& Relay ▸ Failed to launch ${spec.binary}: ${err.message}.\n`);
  ...
  if (detachedSessionActive) {
    process.stderr.write("ai& Relay ▸ Background Claude session remains routed through ai&.\n");
    return result;
  }
...
export function randomLocalProxyToken(): string {
  return `aiandrelay-${randomBytes(24).toString("base64url")}`;
}
