...
type OpenAIChatResponseWithAiandrelayMeta = OpenAIChatResponse & {
  _aiandrelayRequestedMaxTokens?: number;
};

export function thinkingSignature(reasoning: string): string {
  return `aiandrelay:${stableHash(reasoning)}`;
}
...
export function claudeModelResponse(model: ResolvedClaudeModel): Record<string, unknown> {
  return {
    ...
    type: "model",
    object: "model",
    display_name: `ai& ${model.definition.name}`,
...
export function countTokensResponse(
  ...
  // Claude Code polls routinely, and is calibrated against ai&'s real
...
export function toAnthropicMessage(
  ...
  const requestedMaxTokens = (response as OpenAIChatResponseWithAiandrelayMeta)
    ._aiandrelayRequestedMaxTokens;
