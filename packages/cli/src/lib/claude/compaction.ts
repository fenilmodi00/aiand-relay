...
const COMPACTION_SIGNATURES = [
  ...
  // them without tying ai& Relay to one exact release's wording.
...
export function tuneClaudeCompactionRequest(
  ...
  // value, but ai& Relay does not impose a separate compaction-only cap.
...
function boundedCompactionInstruction(
  ...
  return `ai& Relay bounded compaction request:
