...
import { writeAnthropicError } from "./aiand-call.js";
import { streamAnthropicFromAiand } from "./stream.js";
import { extractImageBlocks, resolveImageBlocks } from "./vision-resolver.js";
import { callAiandChatCompletions } from "./chat-completions.js";
...
export type ClaudeProxyOptions = {
  ...
  baseUrl: string;
  /** Claude-facing model alias, e.g. aiand-glm-5-2. */
  modelId: string;
  /** ai& API model id, e.g. zai-org/glm-5.2. */
...
export async function handleProxyRequest(
  ...
  const openAiResponse = await callAiandChatCompletions(
    body,
    {
      ...options,
      rawBytes: budgetRawBytes,
