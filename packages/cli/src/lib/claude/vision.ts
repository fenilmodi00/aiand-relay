...
import { AIAND_BASE_URL, VISION_PROMPT, getVisionModels } from "@aiandrelay/models";
 ...
 * route each image to a vision-capable ai& serverless model, get a text
 ...
 * image it never saw.
 *
 * The vision model list and prompt come from @aiandrelay/models (the shared
...
function visionFailoverRaceDelayMs(): number | undefined {
  const raw = process.env.AIANDRELAY_VISION_FAILOVER_RACE_DELAY_MS;
...
function debug(options: VisionRequestOptions, label: string, value: unknown): void {
  ...
  process.stderr.write(`[aiandrelay vision] ${label}: ${JSON.stringify(value)}\n`);
}
