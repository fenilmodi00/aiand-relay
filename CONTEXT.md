# Domain Glossary - aiandrelay

The ubiquitous language for the aiandrelay CLI proxy/router. Terms here are
...
- **Spawned harness** - OpenCode, Pi, Prime, Hermes, DeepSeek, Grok, omp. `run` spawns the agent binary
...
**HarnessId** - the enum of harness identifiers (`claude`, `codex`, `deepseek`,
`grok`, `opencode`, `pi`, `prime`, `hermes`, `omp`). Note: the daemon also knows
about `codex-app`, an agent id not in `HarnessId` - an orphan to be reconciled.
...
own ai& `apiKey`, model, and `CostTracker`; the daemon owns the session
...
**ai& client** (`aiand-client.ts`) - the deep HTTP client for
`POST /chat/completions`. Owns the fetch + 429/503 retry loop + backoff. Each
harness's `aiand-call.ts` maps the response to its own wire-format error
...
**paths** (`paths.ts`, shared) - the single source of truth for the aiandrelay
